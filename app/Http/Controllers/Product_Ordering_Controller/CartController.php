...
namespace App\Http\Controllers\Product_Ordering_Controller;
    ...
    class CartController extends Controller
    {
        ...
        public function addtocart(Request $request)
        {
            ...
            ### find product id matching database (quantity validator tools)
            $p_id = $request->input('product_id');
            $q_heck = Products::find($p_id);
            $q_checking = $q_heck->quantity;
            #######################################
              ...
              (
                  $request->all(),
                  [
                      'quantity' => ['required', 'integer', 'min:1','max:'.$q_checking],
                  ]
              );
        ...
        if($Validator->fails())
            {
                $Response=$Validator->messages();
                //return response()->json($Response,200);
                return response()->json(['status'=>'You can\'t add 0 items']);
            }
            else
            {
                $prod_id = $request->input('product_id');
